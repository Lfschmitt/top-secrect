...
public class Food : MonoBehaviour {
    ...
    void ReadLevel()
    {
        if (allPoints.food > minPointsForUpdate[0])
        {
            image2 = 1;
            if (allPoints.food > minPointsForUpdate[1])
            {
                image1 = 2;
                if (allPoints.food > minPointsForUpdate[2])
                {
                    image4 = 4;
                    if (allPoints.food > minPointsForUpdate[3])
                    {
                        image2 = 2;
                        if (allPoints.food > minPointsForUpdate[4])
                        {
                            image1 = 3;
                            if (allPoints.food > minPointsForUpdate[5])
                            {
                                image2 = 3;
                                if (allPoints.food > minPointsForUpdate[6])
                                {
                                    image3 = 4;
                                }
                            }
        ...
        {
            if (PlayerPrefs.GetInt("FoodFirstBuy") == 0)
                image1 = 0;
            image2 = 0;
            image3 = 0;
            image4 = 0;
    ...
    public void FirstBuy()
    {
        ...
        {
            image1 = 1;
            PlayerPrefs.SetInt("FoodFirstBuy", 1);
        }
    }
}